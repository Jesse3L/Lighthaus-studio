...
https.get('https://www.lighthausstudio.com/real-estate-photography-lubbock-tx', res => {
...
https.get('https://www.lighthausstudio.com/sitemap.xml', res => {
    let doc = ''; res.on('data', c => doc += c); res.on('end', () => {
        console.log(`Sitemap entry: ${(doc.match(/<loc>.*?lubbock.*?<\/loc>/i) || [])[0]}`);
    });
});

https.get('https://www.lighthausstudio.com/', res => {
    let doc = ''; res.on('data', c => doc += c); res.on('end', () => {
        let match1 = doc.match(/<a[^>]*href="\/real-estate-photography-lubbock-tx"[^>]*>Lubbock, TX<\/a>/i);
        console.log(`Homepage internal link 1 (LocalArea): ${match1 ? match1[0] : 'NOT FOUND'}`);
        
        let match2 = doc.match(/<a[^>]*href="\/real-estate-photography-lubbock-tx"[^>]*>Lubbock and the surrounding West Texas communities<\/a>/gi);
...
https.get('https://www.lighthausstudio.com/services', res => {
    let doc = ''; res.on('data', c => doc += c); res.on('end', () => {
        let match = doc.match(/<a[^>]*href="\/real-estate-photography-lubbock-tx"[^>]*>Lubbock<\/a>/i);
